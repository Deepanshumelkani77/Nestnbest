...
import { getCommercialById } from '../data/commercial'
import {
  MapPin,
  Building2,
  Star,
...
const SIMILAR_COMMERCIAL = [
  { name: 'Sector 62 Office Tower', location: 'Gurgaon', price: '₹15.5 Cr', image: 'https://images.unsplash.com/photo-1486406146926-c627a92ad1ab?w=400&h=300&fit=crop', area: '10,000 sq.ft' },
  { name: 'Golf Course Road Showroom', location: 'Gurgaon', price: '₹18.5 Cr', image: 'https://images.unsplash.com/photo-1555529669-e69e7aa0ba9a?w=400&h=300&fit=crop', area: '7,400 sq.ft' },
  { name: 'Sector 49 Co-working Space', location: 'Gurgaon', price: '₹4.2 Cr', image: 'https://images.unsplash.com/photo-1497366754055-f35944d8ce11?w=400&h=300&fit=crop', area: '3,500 sq.ft' },
]
...
const CommercialDetail = () => {
  ...
  const commercial = getCommercialById(Number(id))
  ...
  const images = commercial.images || [commercial.image, commercial.image, commercial.image, commercial.image, commercial.image]

  // ---- Price trend data (per sq.ft) ----
  const priceTrendData = useMemo(() => {
    return commercial.priceTrend || [
  ...
  const nearby = commercial.nearby || [
    { label: 'Cyber City Metro', distance: '0.2 km', icon: 'metro' },
    { label: 'Ambience Mall', distance: '0.5 km', icon: 'shopping' },
    { label: 'Medanta Hospital', distance: '3.0 km', icon: 'hospital' },
    { label: 'DPS International', distance: '2.5 km', icon: 'school' },
  ]
      ...
      <div className="bg-white border-b border-slate-100">
        <div className="max-w-8xl mx-auto px-4 sm:px-6 lg:px-8 py-3.5">
          <div className="flex items-center gap-2 text-sm text-slate-500">
            ...
            <ChevronDivider />
            <span className="hover:text-[#1E88E5] cursor-pointer transition-colors duration-150">Commercial</span>
            <ChevronDivider />
            <span className="font-medium" style={{ color: NAVY }}>{commercial.name}</span>
          </div>
        </div>
      </div>
      ...
      <div className="max-w-8xl mx-auto px-4 sm:px-6 lg:px-8 pt-6">
        <div className="grid grid-cols-1 lg:grid-cols-[1.6fr_1fr] gap-3 rounded-2xl overflow-hidden">
          <div className="relative h-[280px] sm:h-[420px] rounded-2xl overflow-hidden group">
            <img src={images[activeImage]} alt={commercial.name} className="w-full h-full object-cover" />
            <button
              onClick={() => goToImage(-1)}
          ...
          <div className="grid grid-cols-2 gap-3 h-[280px] sm:h-[420px]">
            {images.slice(1, 5).map((img, i) => (
              <button key={i} onClick={() => setActiveImage(i + 1)} className="relative rounded-xl overflow-hidden h-full">
                <img src={img} alt={`${commercial.name} view ${i + 2}`} className="w-full h-full object-cover hover:scale-105 transition-transform duration-300" />
              </button>
            ))}
          </div>
        </div>
      ...
      <div className="max-w-8xl mx-auto px-4 sm:px-6 lg:px-8 pt-6">
        <div className="bg-white rounded-2xl border border-slate-100 shadow-sm p-6 sm:p-7">
          <div className="flex flex-col lg:flex-row lg:items-start justify-between gap-5">
            <div className="min-w-0">
              <div className="flex flex-wrap items-center gap-2 mb-2">
                <h1 className="text-2xl sm:text-3xl font-bold" style={{ color: NAVY }}>{commercial.name}</h1>
            ...
            <div className="flex items-center gap-3 flex-shrink-0">
              ...
              <div className="text-right">
                <p className="text-2xl font-bold" style={{ color: NAVY }}>{commercial.price}</p>
                <p className="text-slate-400 text-xs">{commercial.pricePerSqft}</p>
              </div>
            </div>
          </div>
          ...
          <div className="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-5 gap-3 mt-6 pt-6 border-t border-slate-100">
            <StatCard icon={Maximize} label="Total Area" value={commercial.area} />
            <StatCard icon={Layers} label="Floors" value={commercial.floors} />
            <StatCard icon={Building2} label="Property Type" value={commercial.propertyType} />
            <StatCard icon={Compass} label="Possession" value={commercial.possession} />
            <StatCard icon={ShieldCheck} label="RERA ID" value={commercial.reraId} />
          </div>
        </div>
      </div>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
          ...
          <div className="lg:col-span-2 space-y-6">
            ...
            <SectionCard eyebrow="Overview" title="About this property">
              <p className="text-slate-600 leading-relaxed">
                {commercial.description ||
                  `Premium ${commercial.propertyType.toLowerCase()} in ${commercial.location}, offering ${commercial.area} of modern commercial space. This property features excellent amenities, prime location, and is ideal for businesses looking for a prestigious address.`}
              </p>
              {commercial.amenities && (
                <div className="flex flex-wrap gap-2 mt-5">
                  {commercial.amenities.map((a) => (
            ...
            <SectionCard eyebrow="Specifications" title="Property details">
              <div>
                <SpecRow icon={Maximize} label="Total Area" value={commercial.area} />
                <SpecRow icon={Layers} label="Floor(s)" value={commercial.floors} />
                <SpecRow icon={Building2} label="Property Type" value={commercial.propertyType} />
                <SpecRow icon={Compass} label="Possession" value={commercial.possession} />
                <SpecRow icon={ShieldCheck} label="RERA ID" value={commercial.reraId} />
                <SpecRow icon={Droplets} label="Water Connection" value="Available" />
                <SpecRow icon={Zap} label="Power Backup" value="100%" />
                <SpecRow icon={Building2} label="Builder" value={commercial.builder} />
              </div>
            </SectionCard>
            ...
            <SectionCard eyebrow="Amenities" title="What this property offers">
              <div className="grid grid-cols-2 sm:grid-cols-3 gap-4">
                {commercial.amenitiesDetailed?.map((a) => (
              ...
              <div className="h-64">
                <ResponsiveContainer width="100%" height="100%">
                  <AreaChart data={priceTrendData} margin={{ top: 10, right: 10, left: -10, bottom: 0 }}>
                    <defs>
                      <linearGradient id="commercialPriceGradient" x1="0" y1="0" x2="0" y2="1">
                    ...
                    />
                    <Area type="monotone" dataKey="price" stroke={BLUE} strokeWidth={2.5} fill="url(#commercialPriceGradient)" />
                  </AreaChart>
                </ResponsiveContainer>
              </div>
              <p className="text-slate-400 text-xs mt-4">
                * Price trends are indicative, based on transacted commercial properties in the surrounding locality over the last 5 years.
              </p>
            </SectionCard>
            ...
            <SectionCard eyebrow="Floor Plans" title="Available configurations">
              <div className="overflow-x-auto -mx-1">
                <table className="w-full text-sm min-w-[420px]">
                  ...
                  <tbody>
                    {commercial.floorPlans?.map((plan) => (
            ...
            <SectionCard eyebrow="Location" title="Location & connectivity">
              <div className="rounded-xl overflow-hidden border border-slate-100 h-64 mb-5">
                <iframe
                  title={`${commercial.name} location`}
                  className="w-full h-full"
                  loading="lazy"
                  referrerPolicy="no-referrer-when-downgrade"
                  src={`https://www.google.com/maps?q=${commercial.coordinates || commercial.location}&z=14&output=embed`}
                />
              </div>
            ...
            <div className="sticky top-6 space-y-6">
              ...
              <div className="bg-white rounded-2xl border border-slate-100 shadow-sm p-6">
                ...
                <div className="flex items-center gap-3">
                  ...
                  <div className="min-w-0">
                    <p className="font-semibold text-sm truncate" style={{ color: NAVY }}>{commercial.builder}</p>
                    <p className="text-slate-400 text-xs flex items-center gap-1 mt-0.5">
                      <Star size={11} fill="#F59E0B" className="text-amber-500" />
                      {commercial.rating} · Verified Builder
                    </p>
                  </div>
                </div>
              </div>
